...
// NB: We can't parameterize these load functions, as esbuild only bundles
// dynamically if the import string itself isn't dynamic

// These template engines are opt-ins, which is which we need to import them
// conditionally
const load_mustache = async _ => {
  try {
    return await import(`htmx-esm/client-side-templates/mustache`);
  } catch (_) {
    console.log(_)
    return undefined;
  }
};
const load_handlebars = async _ => {
  try {
    return await import(`htmx-esm/client-side-templates/handlebars`);
  } catch (_) {
    return undefined;
  }
};
const load_nunjucks = async _ => {
  try {
    return await import(`htmx-esm/client-side-templates/nunjucks`);
  } catch (_) {
    return undefined;
  }
};

const Mustache = (await load_mustache())?.Mustache.default;
const Handlebars = (await load_handlebars())?.Handlebars;
const nunjucks = (await load_nunjucks())?.nunjucks;
